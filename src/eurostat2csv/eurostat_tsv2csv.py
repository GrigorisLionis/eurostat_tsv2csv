...
def eurostat_tsv2csv(table_name,path):
    ...
    if(last_variables[1]!="TIME_PERIOD"):
    ...
    for i in range(0,len(variables)-3):
        sql_string_vars=sql_string_vars+sql_string_cast(variables[i],len_of_fields[i]) 
    sql_string_vars="CREATE TABLE "+table_name+" ("+sql_string_vars
    sql_string_vars=sql_string_vars+sql_string_cast("obs_time",len_of_time)
